# Skripty a hooky

Podľa `playbook/03_kreativa/`. Mobil, vertikál 9:16, titulky napevno, hovorí Miriam.
Dĺžka 15 až 45 sekúnd.

> 🔴 **Každý variant sa točí celý nanovo.** Toto nie sú dva skripty, z ktorých sa vystrihujú
> úvody. Sú to dve **kostry naplnené inými slovami**, iným prostredím a iným dôrazom.
> Ak by si dve videá položil vedľa seba, divák musí na prvý pohľad vidieť rozdiel.

---

## Kostra, ktorú napĺňa každé video

```
[0-3 s]    HOOK       iný v každom variante
[3-10 s]   PROBLÉM    bolesť, ktorú divák pozná
[10-20 s]  RIEŠENIE   čo robí a čím sa to líši
[20-30 s]  DÔKAZ      konkrétny detail zo stavby, nie tvrdenie o sebe
[30-40 s]  PONUKA     checklist zadarmo
[40-45 s]  CTA        klikni na odkaz
```

⚠️ **Dôkaz robíme detailom, nie číslom.** Miriam zatiaľ nemá doložený počet klientok
(viď [`README.md`](README.md)). Konkrétny detail zo stavby, ktorý laik nepozná, dokazuje
odbornosť rovnako dobre a nedá sa spochybniť.

---

# Verzia A: „Okno je o dvadsať centimetrov nižšie"

**Prostredie:** rozostavaný byt alebo holobyt, ideálne pri okne
**Oblečenie:** pracovné, bunda
**Dôraz:** už sa to nedá vrátiť
**Dĺžka:** cca 40 s

> **HOOK (0-3 s)**
> „Toto okno začína o dvadsať centimetrov nižšie, než si tá pani myslela."
>
> **PROBLÉM (3-10 s)**
> „Kupovala byt z papiera. Predstavovala si, že pod oknom bude komoda. Zistila to
> v deň preberania. Komoda tam nesadne a sedačka zakrýva pol okna."
>
> **RIEŠENIE (10-20 s)**
> „Pritom to bolo v podkladoch. Len jej nikto nedal rozvinuté steny, a ona nevedela,
> že si ich má vypýtať. To je jeden riadok v e-maile developerovi."
>
> **DÔKAZ (20-30 s)**
> „Takýchto vecí je v novostavbe asi štyridsať. Revízne dvierka v kúpeľni, ktoré rozbijú
> obklad. Zásuvka za telkou tridsať centimetrov nad podlahou. Šedá lišta cez celý byt."
>
> **PONUKA (30-38 s)**
> „Spísala som ich všetky do checklistu. Čo si vypýtať, a hlavne kedy. Je zadarmo."
>
> **CTA (38-42 s)**
> „Klikni na odkaz pod videom a pošlem ti ho na e-mail."

---

# Verzia B: „Volali mi tri dni po preberaní"

**Prostredie:** doma alebo v ateliéri, pri stole s výkresmi
**Oblečenie:** civilné, sveter
**Dôraz:** termín, ktorý ľuďom ujde
**Dĺžka:** cca 35 s

> **HOOK (0-3 s)**
> „Najsmutnejší telefonát je tri dni po preberaní bytu."
>
> **PROBLÉM (3-11 s)**
> „Vtedy mi ľudia volajú prvýkrát. A ja im musím povedať, že polovicu vecí, ktoré chcú,
> už spraviť nejde. Nie draho. Vôbec."
>
> **RIEŠENIE (11-21 s)**
> „Lebo o všetkom podstatnom sa rozhoduje pri uzávierke klientských zmien. To je termín,
> po ktorom developer prestane brať zmeny. A býva mesiace pred odovzdaním."
>
> **DÔKAZ (21-30 s)**
> „Do betónového stropu sa dodatočne sekať nesmie. Takže ak chceš svetlo nad jedálenským
> stolom a nie je v podklade pre elektrikára, nebude tam nikdy."
>
> **PONUKA (30-36 s)**
> „Spísala som zoznam, čo si od developera vypýtať a dokedy. Pošlem ti ho zadarmo."
>
> **CTA (36-40 s)**
> „Odkaz máš pod videom."

**Prečo sú tie dve verzie naozaj iné:** A je natočená na mieste, ukazuje konkrétnu vec
a tlačí na stratu. B je natočená pri stole, hovorí o termíne a tlačí na čas. Iné prostredie,
iné oblečenie, iný dôraz, iné vety. Presne takto sa robí zvyšných osem.

---

# 8 hookov

Prvé tri sekundy rozhodujú. Preto sú naprieč typmi, nie osemkrát to isté.

| # | Typ | Znenie |
|---|---|---|
| **1** | otázka na bolesť | „Vieš, kedy ti u developera vyprší lehota na zmeny? Väčšina ľudí to zistí neskoro." |
| **2** | oslovenie lokality | „Trnava a okolie, ak si kúpila byt v novostavbe, toto si pozri." |
| **3** | oslovenie cieľovky | „Toto je pre teba, ak preberáš byt v najbližšom roku." |
| **4** | negatívny filter | „Ak už v tom byte rok bývaš, toto video ti nepomôže. Povedzme si prečo." |
| **5** | konkrétne číslo | „Štyridsať vecí, ktoré si máš vypýtať od developera. Tri z nich sú kritické." |
| **6** | ponuka rovno | „Dávam zadarmo zoznam, čo si vypýtať od developera. Poviem ti, prečo ho robím." |
| **7** | námietka | „Myslíš si, že ti developer sám povie, čo potrebuješ? Povie. Ale až keď sa spýtaš." |
| **8** | osobný príbeh | „Pätnásť rokov robím interiéry a stále ma najviac mrzí ten istý telefonát." |

**Ku každému hooku sa píše vlastná verzia tela.** Nie tá istá myšlienka desaťkrát,
ale desať rôznych ciest k tomu istému záveru.

Príklady, ako sa telo mení podľa hooku:

- **Hook 4** (negatívny filter) pokračuje tým, čo sa dá zachrániť aj po uzávierke:
  nenechať si namontovať štandard, ktorý aj tak vyhodíš.
- **Hook 7** (námietka) pokračuje tým, ktoré podklady developer bežne nedá, kým si
  o ne nenapíšeš, teda rozvinuté steny a skladbu podlahy.
- **Hook 5** (číslo) pokračuje troma najkritickejšími vecami zo zoznamu, nie celým zoznamom.

---

# Plán natáčania

Dva kratšie dni sú lepšie než jeden dlhý. Na druhom natáčaní je človek uvoľnenejší
a videá vyzerajú inak, čo je presne to, čo chceme.

| # | Hook | Prostredie | Oblečenie | Dôraz | Deň | Súbor |
|---|---|---|---|---|---|---|
| 1 | 1, otázka na bolesť | holobyt, pri okne | pracovné | strata | 1 | `01_hook-otazka.mp4` |
| 2 | 2, lokalita | pred novostavbou | pracovné | lokálnosť | 1 | `02_hook-lokalita.mp4` |
| 3 | 5, číslo | holobyt, kúpeľňa | pracovné | odbornosť | 1 | `03_hook-cislo.mp4` |
| 4 | 7, námietka | holobyt, chodba | pracovné | dôvera | 1 | `04_hook-namietka.mp4` |
| 5 | 3, cieľovka | schodisko, exteriér | pracovné | konkrétnosť | 1 | `05_hook-cielovka.mp4` |
| 6 | 8, osobný príbeh | ateliér, pri stole | sveter | dôvera | 2 | `06_hook-pribeh.mp4` |
| 7 | 6, ponuka rovno | ateliér, pri výkresoch | sveter | hodnota | 2 | `07_hook-ponuka.mp4` |
| 8 | 4, negatívny filter | doma, pri kávе | sveter | filter | 2 | `08_hook-filter.mp4` |
| 9 | 1 inak, otázka | ateliér | sveter | čas | 2 | `09_hook-otazka-b.mp4` |
| 10 | 5 inak, číslo | doma | sveter | odbornosť | 2 | `10_hook-cislo-b.mp4` |

**Minimum je 10 videí.** Ideál 15 až 30. Menej znamená, že algoritmus nemá z čoho vyberať
a rozdiely medzi reklamami budú šum, nie signál.

---

## Pravidlá pre natáčanie

| Vec | Ako |
|---|---|
| **Zvuk** | Najdôležitejšie zo všetkého. Ticho alebo mikrofón na klope. Na stavbe je ozvena, treba to skúsiť vopred. |
| **Svetlo** | Oknom do tváre. Nikdy okno za chrbtom, v holobyte je to najčastejšia chyba. |
| **Kompozícia** | Tvár v hornej tretine, oči do objektívu. |
| **Prvý záber sa musí hýbať** | Statická hovoriaca hlava od nuly stráca pozornosť. Začni pohybom kamery alebo krokom. |
| **Energia** | O niečo živšie než v bežnom rozhovore. |
| **Strih** | Rezy každé 3 až 5 sekúnd. |
| **Titulky** | Napevno, veľké. Väčšina ľudí pozerá bez zvuku. |
| **Nečítať z papiera** | Skript sa naučí, nepredčíta. Zaváhania sú v poriadku, znejú autenticky. |

⚠️ **Amatérske neznamená odflaknuté.** Amatérske video funguje preto, že vyzerá ako bežný
príspevok, nie ako reklama. Ale musí byť počuť, vidieť a musí mať napísaný skript.

---

## Čo Meta odmietne

Pri tomto odbore je riziko malé, ale pre istotu:

- text cez viac než zhruba 20 % plochy videa
- hudba bez licencie
- záber, kde je vidieť tvár človeka, ktorý nedal súhlas

Zdravotné a finančné sľuby tu nehrozia, ale **nesľubuj konkrétnu úsporu v eurách**,
kým to nie je doložené.
